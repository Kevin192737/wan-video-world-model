#!/usr/bin/env python3
"""Batch resample *_joint.txt (same CSV layout as action.txt) to a fixed row count."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch resample joint.txt-style CSV files to target steps "
        "(linear interpolation along time, same format as tools/action_head/csv_action)."
    )
    parser.add_argument(
        "--input_dir",
        type=Path,
        default=Path("/home/release/pred_joints_batch"),
        help="Directory containing *_joint.txt (default: /home/release/pred_joints_batch).",
    )
    parser.add_argument(
        "--output_dir",
        type=Path,
        required=True,
        help="Directory to write resampled joint txt files.",
    )
    parser.add_argument(
        "--target_steps",
        type=int,
        default=51,
        help="Target number of trajectory rows excluding header (default: 51, matches sample_result/*/joint.txt).",
    )
    parser.add_argument(
        "--glob",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override output index column start. Default: keep each file's original first index.",
    )
